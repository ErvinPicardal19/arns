...
model = Model(r"/home/ervinpicardal/dev_ws/src/arns_voice_cmd/vosk-model-en")
recognizer = KaldiRecognizer(model, 16000)
mic = pyaudio.PyAudio()
stream = mic.open(format=pyaudio.paInt16, channels=1,
                     rate=16000, input=True, frames_per_buffer=8192)
sound_files_path = '/home/ervinpicardal/dev_ws/src/arns_voice_cmd/resource'
...
class Voice_CMD_Node(Node):
   def __init__(self):
      ...
      self.cmd_vel_publisher_ = self.create_publisher(Twist, "/diff_controller/cmd_vel_unstamped", 10)
   ...
   def respond(self, voice_data):
      if not self.wake_up:
         ...
         if 'kitchen' in voice_data.lower():
            ...
            self.get_logger().info("Going to the living room")
